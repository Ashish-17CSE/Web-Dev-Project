...
window.addEventListener("DOMContentLoaded", () => {
    axios.get('https://crudcrud.com/api/4c506e2ae3cb4a32aeff6fb717c5d64b/formData')
        .then((response) => {
...
function readDate() {
    ...
    axios.post('https://crudcrud.com/api/4c506e2ae3cb4a32aeff6fb717c5d64b/formData', user)
        .then((response) => {
...
function update() {
    let userId = editdata.cells[4].innerHTML; // get the user ID from the row
    let user = {
    ...
    axios
        .put(`https://crudcrud.com/api/4c506e2ae3cb4a32aeff6fb717c5d64b/formData/${userId}`, user)
        .then((response) => {
...
function dlt(dl) {
    let userId = dl.parentElement.parentElement.cells[4].innerHTML; // get the user ID from the row
    axios
        .delete(`https://crudcrud.com/api/4c506e2ae3cb4a32aeff6fb717c5d64b/formData/${userId}`)
        .then((response) => {
            console.log("Delete Data: ", response);
            alert("Data deleted successfully");
            dl.parentElement.parentElement.remove(); // remove the row from the table
        })
        .catch((err) => {
            console.log(err);
        });
